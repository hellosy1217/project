...
public class BoardController {
	...
	public ModelAndView boardList(HttpServletRequest req, @RequestParam(defaultValue = "1") int curPage,
		...
		for (int i = 0; i < bestReview.size(); i++) {
			for (int j = 0; j < listMember.size(); j++) {
				if (bestReview.get(i).getMember_email() == listMember.get(j).getMember_email()) {
					bestReview.get(i).setBoard_memberImg(listMember.get(j).getMember_str_img());
					System.out.println("멤버이미지 = "+ bestReview.get(i).getBoard_memberImg());
					memberImg = bestReview.get(i).getBoard_memberImg();
					if(memberImg == null || memberImg.trim().equals("")) {
						memberImg = "N";
					}else {
						memberImg = "Y";
					}
		...
		for (int i = 0; i < bestReview.size(); i++) {
			for (int j = 0; j < listPack.size(); j++) {
				if (bestReview.get(i).getPack_num() == listPack.get(j).getPack_num()) {
					bestReview.get(i).setBoard_packImg(listPack.get(j).getPack_str_img());
				}
			}
		}
		...
		Listmap.put("memberImg", memberImg);
	...
	public ModelAndView contentBoard(HttpServletRequest req, HttpServletResponse resp, @RequestParam String board_num) {
		...
		int packNum = boarddto.getPack_num();

		String p_name = null;
		String str_img = null;
		int prod_pick = 0;
		if(prodNum != 0 && packNum == 0) { //상품에 대한 후기 일때
			//상품 이미지 & 이름
			List<BBProductDTO> listProd = ProductMapper.listProd();
			for (int j = 0; j < listProd.size(); j++) {
				if (prodNum == listProd.get(j).getProd_num()) {
					p_name = listProd.get(j).getProd_name();
					str_img = listProd.get(j).getProd_str_img();
					prod_pick = listProd.get(j).getProd_pick();
				}
			}  
		}else if(packNum != 0 && prodNum == 0) { //패키지에 대한 후기일때
			List<BBPackageDTO> listPack = packageMapper.listPack();
			for (int j = 0; j < listPack.size(); j++) {
				if (packNum == listPack.get(j).getPack_num()) {
					p_name = listPack.get(j).getPack_title();
					str_img = listPack.get(j).getPack_str_img();
				}
			}  
		}
		...
		String memberImg = boarddto.getBoard_memberImg();
		if(memberImg == null || memberImg.trim().equals("")) {
			memberImg = "N";
		}else {
			 memberImg = "Y";
		...
		if(pnum != 0) {
			for (int j = 0; j < listProd.size(); j++) {
				if (pnum == listProd.get(j).getProd_num()) {
					pname =listProd.get(j).getProd_name();
					str_img = listProd.get(j).getProd_str_img();
					prod_pick = listProd.get(j).getProd_pick();
				}
			}
			...
			for (int j = 0; j < listPack.size(); j++) {
				if (packNum == listPack.get(j).getPack_num()) {
					pname = listPack.get(j).getPack_title();
					str_img = listPack.get(j).getPack_str_img();
				}
			}
		}
